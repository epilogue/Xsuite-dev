...
//                var_dump($year) ;
    ...
    public function verifreferenceAction(){
        ...
        $requete1 ="select MITMAS.MMITNO from EIT.MVXCDTA.MITMAS MITMAS where  RTRIM(MITMAS.MMITDS) ='{$reference_article}' and  MITMAS.MMCONO='100'";
//        var_dump($requete1);
...
//            exit();
           ...
           // var_dump($idclientuser);
            /*insertion en bdd pour la table client_user_xprev*/
             ...
             $dataclient = array(
                 'tracking'=>$trackingnumber,
                 'code_user_client_xprev'=>$idclient[0]['code_client'],
                 'nom_client_xprev'=>$idclient[0]['nom_client']  
             );
...
//                 $params['destinataireMail']="mhuby@smc-france.fr";
                ...
                $redirector = $this->_helper->getHelper('Redirector');
                $flashMessenger = $this->_helper->getHelper('FlashMessenger');
                $message = "la demande de prévision a bien été refusée.{$infoPrev[0]['email_emetteur']}";
                $flashMessenger->addMessage($message);
                $redirector->gotoSimple('index', 'xprev');
            }
        }
...
//                 $params['destinataireMail']="mhuby@smc-france.fr";
            ...
            elseif ($formData['validlog']=='2') {
             /*si demande d'info envoi au createur */
                $statut=1;
                $validation =4;
                $justification = $formData['motif_validation'];
...
//                 $params['destinataireMail']="mhuby@smc-france.fr";
                ...
                $redirector = $this->_helper->getHelper('Redirector');
                $flashMessenger = $this->_helper->getHelper('FlashMessenger');
                $message = "demande de complément d'information pour la demande de prévision a bien été envoyée.{$infoPrev[0]['email_emetteur']}";
                $flashMessenger->addMessage($message);
                $redirector->gotoSimple('index', 'xprev');
            }
        }
