...
class Application_Model_DbTable_Xdistrib extends Zend_Db_Table_Abstract {
    ...
    public function searchByCDR($tracking1,$tracking2){
     ...
     public function searchforDBD(){
        $sql="SELECT demande_xdistrib.id_demande_xdistrib, validations_demande_xdistrib.nom_validation, `num_workplace_demande_xdistrib`,demande_xdistrib. tracking_number_demande_xdistrib,users.nom_user,  `date_demande_xdistrib`, demande_xdistrib. id_user, client_distrib.nom_client,validations_demande_xdistrib.etat_validation  FROM `demande_xdistrib` 
join users on users.id_user=demande_xdistrib.id_user
join client_distrib on client_distrib.numwp_client = demande_xdistrib.numwp_client
