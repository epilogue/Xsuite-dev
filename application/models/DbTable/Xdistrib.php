...
JOIN validations_demande_xdistrib ON validations_demande_xdistrib.id_demande_xdistrib = demande_xdistrib.`id_demande_xdistrib`
GROUP BY demande_xdistrib.`tracking_number_demande_xdistrib` order by demande_xdistrib.`id_demande_xdistrib` desc) and validations_demande_xdistrib.`id_demande_xdistrib`={$key} order by demande_xdistrib.`id_demande_xdistrib` desc";
   $res = $this->getAdapter()->query($sql);
        $rest=$res->fetchAll();
        if (!$rest) {
            return null;
    ...
    public function essaiTest($id_dd){
        $sql="select tracking_number_demande_xdistrib,
            num_workplace_demande_xdistrib,
            id_demande_xdistrib,
            id_user
                  from  demande_xdistrib where id_dd";
    }
}
