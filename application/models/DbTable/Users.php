...
class Application_Model_DbTable_Users extends Zend_Db_Table_Abstract {
    ...
    $name="$name";
    $sql ="select zones.nom_zone,nom_user,prenom_user,email_user,id_user,tel_user from users join zones on zones.id_zone=users.id_zone where nom_user like '{$name}%' and id_fonction= 6 ";
      $res = $this->getAdapter()->query($sql);
       $rest = $res->fetchObject();
        if (!$rest) {
            return null;
