...
class Application_Form_NumwpDistrib extends Zend_Form {

    public function init() {
        $this->setMethod('post');
        $this->setAction('/xdistrib/numwp');
        ...
        $num_offre_workplace->setLabel("numéro d'offre workplace : ")
                ->setRequired(TRUE)
                ->setDescription("exemple: 0090XXXXXXX")
                ->addFilter('StripTags')
                ->addFilter('StringTrim')
                ->addValidator('regex', false, array('/^00[9][0][0-9][0-9]/', 'messages' => array(Zend_Validate_Regex::NOT_MATCH => "numéro invalide")))
